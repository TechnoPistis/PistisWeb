...
import { Product, VariantOption, Variant, VariantModel, ProductVariantDetails, ProductVariantOptions, ProductSpecification, ProductImages } from '../../product.model';
...
export class ProductComponent implements OnInit {
  ...
  addProduct(form: NgForm) {
    ...
    this.variantModelArray.forEach(element => {
      ...
      data.Commission=this.Commission1;
...
this.PriceAfterCommission1=0;
  ...
  Commission1: number = 0;PriceAfterCommission1: number = 0;
  ...
  async saveVariants(form: NgForm) {
    ...
    if (this.editIndex >= 0) {
      ...
      if (this.options.length !== this.edit_vari.length || (!Number(this.variant_price))
       ...
       (!Number(this.Lenght)) || (!Number(this.Width)) || (!Number(this.Height))
        ) {
        this.toastr.clear();
        this.toastr.error('Fill All fields to add new variants!');
        return false;
  ...
  CalculatePriceAfterCommission(Commission, SellingPrice, action: string) {
    ...
    if (parseInt(SellingPrice)) {
      if (!Commission)
      Commission = 0;
      let comm=(+SellingPrice*Commission)/100;
