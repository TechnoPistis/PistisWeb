...
this.getFailedTransactions()
  ...
  getFailedTransactions() {
    this.tracklog.getFailedMobileTransactions().subscribe(data => {
      debugger
...
this.tracklog.getMObileFailedTransactions(newDate,value).subscribe(x=>{
  ...
  debugger
      ...
      debugger
      this.tracklog.getPageDateMobileFilter(newDate,value,this.pageName).subscribe(x=>{
        debugger
       // if (x != null) localStorage.setItem('failedTransactions', JSON.stringify(x))
      //  Highcharts.chart("transactionStatus", JSON.parse(localStorage.getItem('failedTransactions')))
      ...
      debugger
    var value=+event.target.value
    this.tracklog.filterMObileUserLogs(this.pageName,this.actionName,newDate,value).subscribe(x=>{
      debugger
...
var value=+event.target.value
if(this.firstGraph==true){
this.tracklog.getFilterAccordingRequestMobile(value).subscribe(x=>{
  if (x != null) localStorage.setItem('failedTransactions', JSON.stringify(x))
  Highcharts.chart("transactionStatus", JSON.parse(localStorage.getItem('failedTransactions')))
    ...
    debugger
    this.tracklog.getFilterAccordingRequestandpageMobile(value,this.pageName).subscribe(x=>{
  ...
  getBestSellers(pageName:string) {
    this.tracklog.getHomeMobileActions(pageName).subscribe(data => {
      debugger
...
onClick2(event:any){
 ...
 this.actionName=event.point.name
  this.tracklog.getMobileActionData(this.pageName,this.actionName).subscribe(x=>
    {
